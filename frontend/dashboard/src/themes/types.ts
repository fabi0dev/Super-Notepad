...
export interface ThemeLayout {
  ...
  radius: string;
  /** Default gap between layout elements. Example: `"0.5rem"`, `"1rem"`. */
  gap: string;
...
export interface DashboardTheme {
   ...
   *  on theme switch. Intended for selsn-level chrome that's too
...
// Dashboard theme selection is client-side only (dark/light/system) via
// localStorage — no server registry response types here.
